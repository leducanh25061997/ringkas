...
export default function SearchByDropdown(props: Props) {
  ...
  const dropItems = [
    { label: 'Type and Unit', value: 'PROPERTY_TYPE_AND_UNIT' },
  ];

  const handleCloseDropdown = () => setOpen(false);
  const handleOpenDropdown = () => setOpen(prev => !prev);

  const handleClickDropItem = (value: any) => {
    setOpen(false);
    onChange(value);
  };

  function convertLabel(text: string) {
    switch (text) {
      case 'PROJECT_NAME':
        return 'project name';
      case 'PROPERTY_TYPE_AND_UNIT':
    ...
    <ClickAwayListener onClickAway={handleCloseDropdown}>
      <SearchBy>
        <div className="drop-btn" onClick={handleOpenDropdown}>
          <div>{value ? convertLabel(value) : 'Search by'}</div>
        ...
        {open ? (
          <DropItems>
            {dropItems.map(item => (
              <DropItem
                onClick={() => handleClickDropItem(item.value as any)}
                key={item.label}
