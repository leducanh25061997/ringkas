...
export default function UpdateEmployeeAccount() {
  ...
  const handleBack = () => {
    navigate(`${path.employeeAccountList}/${id}`);
  };
  ...
  const onSubmit = (inputData: EmployeeAccountDataForm) => {
    ...
    if (inputData.type === EmployeeType.HEAD_QUARTER_LEVEL) {
      inputData.projectId = '';
      inputData.roleName = RoleName.ADMIN;
    }
    inputData.id = id;
    dispatch(
  ...
  useEffect(() => {
    if (employeeAccountInfo) {
      ...
      methods.setValue('type', employeeAccountInfo.type);
      if (
        employeeAccountInfo?.project?.name &&
        employeeAccountInfo?.project.projectId
      ) {
        const projectData = {
          label: employeeAccountInfo?.project.name,
          value: employeeAccountInfo?.project.projectId,
        };
        methods.setValue('projectName', projectData);
      }
      let roleName = {
        label: t(translations.roles.adminProject),
        value: RoleName.ADMIN_PROJECT,
      };
      if (employeeAccountInfo.roleName === RoleName.PIC_PROJECT) {
        roleName = {
          label: t(translations.roles.picProject),
          value: RoleName.PIC_PROJECT,
        };
      }
      if (employeeAccountInfo.type === EmployeeType.HEAD_QUARTER_LEVEL) {
        roleName = {
