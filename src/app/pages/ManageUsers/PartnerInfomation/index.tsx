...
import PartnerInformationDetail from './Component/PartnerInformationDetail';
...
import { selectPartnerInformationDetail } from './Component/selectors';
import { useNavigate } from 'react-router-dom';
import { ApplicationStatus } from 'types';
import { usePartnerInformationSlice } from './Component';
...
const PartnerInformation = React.memo((props: Props) => {
  const params = useParams();
  const { id } = params;
  const { actions } = usePartnerInformationSlice();
  const navigate = useNavigate();
  const { partnerInformation } = useSelector(selectPartnerInformationDetail);
  ...
  const links = React.useMemo(
    () => [
      ...
      {
        title: t(translations.sidebar.partnerAccount),
    ...
    () => [
      {
        ...
        component: <AccountInformation info={partnerInformation} />,
      },
      {
        key: 1,
        title: 'Partner Information',
        component: <PartnerInformationDetail info={partnerInformation} />,
      },
      {
        key: 2,
        title: 'KYC',
        component: <KYC info={partnerInformation} />,
      },
      {
        key: 3,
        title: 'Document Qualification',
        component: <DocumentQualification info={partnerInformation} />,
      },
      {
        key: 4,
        title: 'History log',
  ...
  const handUpdate = () => {
    navigate(`${path.partnerAccountList}/edit/${id}`);
  };
  ...
  const handleVerify = () => {
    ...
    dispatch(
      actions.updateVerificationStatusPartnerAccount(payload, () =>
        setOpenDialog(prev => !prev),
      ),
    );
  };
  ...
  return (
    ...
    >
      ...
      {step !== 2 ||
      partnerInformation?.verificationStatus === ApplicationStatus.VERIFIED ? (
        <ActionButtonWrap>
