...
export default function UploadImageV2(props: Props) {
  ...
  const handleClickUpload = () => {
    ...
    input.onchange = e => {
      ...
      if (files.length + _mediaList.length > 10) {
        setError('Maximum is 10 images');
        return;
      }
      // @ts-ignore
      for (let i = 0; i < files.length; i++) {
        if (files[i].size / 1024 / 1024 > 5) {
          setError(t(translations.imageUploadComponent.maxFileSize));
          return;
        } else {
          _mediaList.push({
            file: files[i],
  ...
  return (
    <>
      {mediaList && mediaList.length > 0 && (
        <div className="p-6 border border-[#D7E2EE] rounded-[8px]">
...
export const ControllerUploadImageV2 = (props: FieldProps) => {
