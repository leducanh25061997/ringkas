...
import { useRegisterBankHQSlice } from './slice';
...
export function RegisterNewBankHQ() {
  const { t } = useTranslation();
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { actions } = useRegisterBankHQSlice();
  ...
  const handleCreateBankHq = () => {
    ...
    dispatch(
      actions.createBankHq(data, (error?: any) => {
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-85px)] pt-4">
      <div className="mb-6 flex items-center px-6">
        ...
        <PageHeader
          title={t(translations.registerNewPartnerBankHq.title)}
          parentItems={[
            {
